 ...
 * to access USB Audio Class (UAC) devices and USB Human Interface Devices (HID)
 * simultaneously.
 *
 * The application showcases the following features:
 * - USB Host stack initialization (EHCI / OHCI)
 * - Enumeration of USB Audio Class and HID Class devices
 * - Audio streaming via ISOCHRONOUS endpoints
 *   - Audio IN (recording) with user callback
 *   - Audio OUT (playback) with user callback
 * - USB Audio Class control requests, including:
 *   - Channel number and bit resolution query
 *   - Sampling rate query and configuration
 *   - Mute and volume control
 *   - Automatic gain control
 * - HID device initialization and interrupt IN handling
 *
 * Audio data is transferred using isochronous pipes, and control/status
 * information is reported through UART0. The sample also demonstrates
 * run-time volume adjustment through console input.
 *
 * @dependency
 * - NuMicro USB Host Library
 * - USB Host UAC Class Driver
 * - USB Host HID Class Driver
 *
 * @note
 * - Audio buffers and USB DMA buffers must be aligned and non-cacheable.
 * - ISO transfer performance may be affected if excessive UART output is used.
 * - This sample is intended for verification, testing, and reference design.
 *
 * @copyright
...
int32_t main(void)
{
    ...
    while (1)
    {
        ...
        /*
         * --------------------------------------------------------------------------
         * User console command handling (UART input control)
         * --------------------------------------------------------------------------
         * This code section monitors user input from the UART console and performs
         * run-time control and status query for the USB Audio Class (UAC) device.
         *
         * Supported commands:
         *
         *  '+' : Increase microphone volume by one resolution step.
         *        - The new volume value is calculated as (vol_cur + vol_res).
         *        - Volume is limited by the maximum supported value (vol_max).
         *        - The new volume is applied by UAC SET_CUR Volume Control request.
         *
         *  '-' : Decrease microphone volume by one resolution step.
         *        - The new volume value is calculated as (vol_cur - vol_res).
         *        - Volume is limited by the minimum supported value (vol_min).
         *        - The new volume is applied by UAC SET_CUR Volume Control request.
         *
         *  '0' : Query and display the current microphone volume.
         *        - The volume is retrieved by UAC GET_CUR Volume Control request.
         *
         * Other keys:
         *        - Display current audio streaming statistics, including:
         *          * Total audio-in data count
         *          * Total audio-out data count
         *        - Dump USB host memory usage information for debugging.
         * --------------------------------------------------------------------------
         */
        if (!kbhit())
        {
