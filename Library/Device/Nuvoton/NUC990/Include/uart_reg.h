...
typedef struct
{
    ...
    * |        |          |Clear Break Error Flag BIF or Frame Error Flag FEF or Parity Error Flag PEF by writing u201C1u201D to corresponding BIF, FEF and PEF to make UART PDMA transmit request operation continue.
    ...
    * |        |          |Clear Break Error Flag BIF or Frame Error Flag FEF or Parity Error Flag PEF by writing u201C1u201D to corresponding BIF, FEF and PEF to make UART PDMA receive request operation continue.
    ...
    * |        |          |0100 = RX FIFO Interrupt Trigger Level is 30 bytes.
    * |        |          |Others = Reserved.
    ...
    * |        |          |0100 = nRTS Trigger Level is 30 bytes.
    * |        |          |Others = Reserved.
    ...
    * |        |          |10 = 7 bits.
    * |        |          |11 = 8 bits.
    * |[2]     |NSB       |Number of u201CSTOP Bitu201D
    * |        |          |0 = One u201CSTOP bitu201D is generated in the transmitted data.
    * |        |          |1 = When select 5-bit word length, 1.5 u201CSTOP bitu201D is generated in the transmitted data
    * |        |          |When select 6-, 7- and 8-bit word length, 2 u201CSTOP bitu201D is generated in the transmitted data.
    ...
    * |        |          |0 = Odd number of logic 1u2019s is transmitted and checked in each word.
    * |        |          |1 = Even number of logic 1u2019s is transmitted and checked in each word.
    ...
    * |        |          |Note 2: This bit is valid when FUNCSEL (UART_FUNCSEL[1:0]) is select UART or RS485 function.
    ...
    * |        |          |Note 1: Refer to Figure 1.1-14 and Figure 1.1-15 for UART function mode.
    * |        |          |Note 2: Refer to Figure 1.1-25 and Figure 1.1-26 for RS-485 function mode.
    ...
    * |        |          |Note: This bit can be cleared by writing u201C1u201D to it.
    ...
    * |        |          |If the number of bytes of received data is greater than RX_FIFO (UART_DAT) size 32 bytes, this bit will be set.
    * |        |          |0 = RX FIFO is not overflow.
    * |        |          |1 = RX FIFO is overflow.
    * |        |          |Note: This bit can be cleared by writing u201C1u201D to it.
    * |[1]     |ABRDIF    |Auto-baud Rate Detect Interrupt Flag
    * |        |          |This bit is set to logic u201C1u201D when auto-baud rate detect function is finished.
    ...
    * |        |          |This bit is set to logic u201C1u201D in Auto-baud Rate Detect mode when the baud rate counter is overflow.
    * |        |          |0 = Auto-baud rate counter is underflow.
    * |        |          |1 = Auto-baud rate counter is overflow.
    * |        |          |Note: This bit can be cleared by writing u201C1u201D to it.
    * |[3]     |ADDRDETF  |RS-485 Address Byte Detect Flag
    * |        |          |0 = Receiver detects a data that is not an address bit (bit 9 =u20190u2019).
    * |        |          |1 = Receiver detects a data that is an address bit (bit 9 =u20191u2019).
    ...
    * |        |          |Note 2: This bit can be cleared by writing u201C1u201D to it.
    ...
    * |        |          |This bit is set to logic 1 whenever the received data input (RX) is held in the u201Cspacing stateu201D (logic 0) for longer than a full word transmission time (that is, the total time of u201Cstart bitu201D + data bits + parity + stop bits).
    ...
    * |        |          |The Maximum value shown in RXPTR is 32
    * |        |          |When the using level of RX FIFO Buffer equal to 32, the RXFULL bit is set to 1
    * |        |          |As one byte of RX FIFO is read by CPU, the RXFULL bit is cleared to 0 and RXPTR will show 31.
    ...
    * |        |          |1 = RX FIFO is full.
    * |        |          |Note: This bit is set when the number of usage in RX FIFO Buffer is equal to 32, otherwise it is cleared by hardware.
    ...
    * |        |          |The Maximum value shown in TXPTR is 32
    * |        |          |When the using level of TX FIFO Buffer equal to 32, the TXFULL bit is set to 1
    * |        |          |As one byte of TX FIFO is transferred to Transmitter Shift Register, the TXFULL bit is cleared to 0 and TXPTR will show 31.
    ...
    * |        |          |1 = TX FIFO is full.
    * |        |          |Note: This bit is set when the number of usage in TX FIFO Buffer is equal to 32, otherwise it is cleared by hardware.
    ...
    * |        |          |Note 1: In RS-485 function mode, this field is set include u201Creceiver detect and received address byte character (bit9 = u20181u2019) bit"
    ...
    * |        |          |Note 1: This bit can be cleared by writing u201C1u201D to it.
    * |        |          |Note 2: If BITOMEN (UART_TOUT[31]) is reset to 0, user can read UART_DAT (RX is in active) to clear this bit.
    * |        |          |Note 3: This bit is valid only when RXPDMAEN (UART_INTEN[15]) and TXPDMAEN (UART_INTEN[14]) are both 0.
    ...
    * |        |          |Note 1: In RS-485 function mode, this field include u201Creceiver detect any address byte received address byte character (bit9 = u20181u2019) bit".
    ...
    * |[27]    |TOFLUSHE  |RX PDMA Time-out Flush Enable Bit
    * |        |          |This bit can enable or disable RX PDMA time-out flush service.
    * |        |          |When TOFLUSHE (UART_TOUT[27]) is enabled, the UART will issue a flush request to the PDMA upon a time-out event
    * |        |          |The PDMA then flushes all data present in the UART RX FIFO at the time of the time-out into the PDMA, and subsequently flushes the data from the PDMA FIFO into system memory.
    * |        |          |0 = RX PDMA time-out flush Disabled.
    * |        |          |1 = RX PDMA time-out flush Enabled.
    * |        |          |Note: This bit has effect only when BITOMEN (UART_TOUT[31]) and TOCNTEN (UART_INTEN[11]) are both set.
    ...
    * |        |          |Note: This bit is read only, but it can be cleared by writing u201C1u201D to ABRDTOIF (UART_FIFOSTS[2]) and ABRDIF(UART_FIFOSTS[1])
    ...
    * |        |          |Note 1: If WKCTSEN (UART_WKCTL[0]) is enabled, the nCTS wake-up cause this bit is set to u20181u2019.
    * |        |          |Note 2: This bit can be cleared by writing u20181u2019 to it.
    * |[1]     |DATWKF    |Incoming Data Wake-up Flag
    * |        |          |This bit is set if chip wake-up from power-down state by data wake-up.
    * |        |          |0 = Chip stays in power-down state.
    * |        |          |1 = Chip wake-up from power-down state by Incoming Data wake-up.
    * |        |          |Note 1: If WKDATEN (UART_WKCTL[1]) is enabled, the Incoming Data wake-up cause this bit is set to u20181u2019.
    * |        |          |Note 2: This bit can be cleared by writing u20181u2019 to it.
    ...
    * |        |          |Note 1: If WKRFRTEN (UART_WKCTL[2]) is enabled, the Received Data FIFO Reached Threshold wake-up cause this bit is set to u20181u2019.
    * |        |          |Note 2: This bit can be cleared by writing u20181u2019 to it.
    ...
    * |        |          |Note 1: If WKRS485EN (UART_WKCTL[3]) is enabled, the RS-485 Address Match (AAD mode) wake-up cause this bit is set to u20181u2019.
    * |        |          |Note 2: This bit can be cleared by writing u20181u2019 to it.
    ...
    * |        |          |Note 1: If WKTOUTEN (UART_WKCTL[4]) is enabled, the Received Data FIFO reached threshold time-out wake-up cause this bit is set to u20181u2019.
    * |        |          |Note 2: This bit can be cleared by writing u20181u2019 to it.
    * @var UART_T::DWKCOMP
...
#define UART_TOUT_TOFLUSHE_Pos           (27)                                              /*!< UART_T::TOUT: TOFLUSHE Position        */
#define UART_TOUT_TOFLUSHE_Msk           (0x1ul << UART_TOUT_TOFLUSHE_Pos)                 /*!< UART_T::TOUT: TOFLUSHE Mask            */
