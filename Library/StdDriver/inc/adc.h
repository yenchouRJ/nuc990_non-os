...
#define ADC_HIGH_SPEED_MODE     ADC_CONF_HSPEED_Msk /*!< ADC working in high speed mode (3.2MHz <= ECLK <= 16MHz) \hideinitializer */
...
/**
  ...
  * \hideinitializer
  */
#define ADC_POWER_DOWN(adc) ((adc)->CTL &= ~ADC_CTL_AD_EN_Msk)
...
/**
  ...
  * \hideinitializer
  */
#define ADC_POWER_ON(adc) ((adc)->CTL |= ADC_CTL_AD_EN_Msk)
...
/**
  ...
  * \hideinitializer
  */
#define ADC_CONVERT_XY_MODE(adc) do {(adc)->CTL &= ~ADC_CTL_PEDE_EN_Msk;\
                                     (adc)->CONF |= ADC_CONF_T_EN_Msk | ADC_CONF_Z_EN_Msk;} while(0)
...
/**
  ...
  * \hideinitializer
  */
#define ADC_DETECT_PD_MODE(adc) do {(adc)->CONF &= ~(ADC_CONF_T_EN_Msk | ADC_CONF_Z_EN_Msk);\
                                    (adc)->CTL |= ADC_CTL_PEDE_EN_Msk;} while(0)
