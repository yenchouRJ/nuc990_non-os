 ...
 * @file     usbd.c
 * @version  V1.00
 * @brief    NuMicro USBD driver source file
 *
 * @note
...
{
    USBD->OPER = (USBD_OPER_HISPDEN_Msk | USBD_OPER_HISHSEN_Msk);   /* high-speed */
    USBD_CLR_SE0();
}
        ...
        case CLEAR_FEATURE:
        {
            if((gUsbCmd.wValue & 0xff) == FEATURE_ENDPOINT_HALT)
            {
                ...
                for (i=0; i<USBD_MAX_EP; i++)
                {
                    ...
                    {
                        USBD->EP[i].EPRSPCTL = (USBD->EP[i].EPRSPCTL & 0xef) | USBD_EP_RSPCTL_TOGGLE;
                    }
                }
            }
            /* Status stage */
    ...
    case SET_CONFIGURATION:
    {
        if (g_usbd_UsbConfig == 0)
        {
            ...
            for (i=0; i<USBD_MAX_EP; i++)
            {
                ...
                {
                    USBD->EP[i].EPRSPCTL = USBD_EP_RSPCTL_TOGGLE;
                }
            }
        }
        break;
